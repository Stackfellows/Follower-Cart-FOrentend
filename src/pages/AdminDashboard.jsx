...
import {
  ...
  XCircle, // For error action messages (added this import)
...
const AdminDashboard = ({ user }) => {
    ...
    try {
      ...
      const formatted = fetchedOrders.map((order) => {
        return {
          ...
          // Ensure date is a valid date string or Date object for parsing
      ...
      const currentMonthRevenue = formatted.reduce(
        (sum, order) =>
          sum + (parseFloat(order.price?.replace("PKR ", "")) || 0),
        0
      );
      setTotalRevenuePKR(currentMonthRevenue.toFixed(2));
      ...
      setOrderStatusChartData([
        ...
        { name: "Pending", count: pending, fill: "#F59E0B" }, // yellow-500
  ...
  const fetchPayments = async () => {
    ...
    try {
      // Corrected: Use the actual backend endpoint for fetching payments
      const res = await axios.get(
        "http://localhost:5000/followerApi/allPayments"
      );
      console.log("Raw payments response:", res.data);
      setPaymentsData(res.data.payments || []); // Assuming your payments are in a 'payments' key
      setActionMessage(null); // Clear any previous action messages on successful fetch
    } catch (error) {
  ...
  const getStatusBadgeClass = (status) => {
    switch (status) {
      ...
      case "Cancelled":
      case "Failed":
      case "Refunded": // Added for consistency with backend enum
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {orders.map((order) => {
                          ...
                          return (
                            ...
                            >
                              <td className="py-3 px-4">
                                {/* Display full order ID */}#{order.orderId}
                              </td>
